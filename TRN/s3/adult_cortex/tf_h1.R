...
J1 <- read.table('your-working-directory/h1.tsv',sep='\t',header = T)
...
tf_j <- cbind(trn_footprint[tf_j$queryHits, c("chr","start","stop","strand" )], J1[tf_j$subjectHits, c('motif_id') ])
colnames(tf_j)[5] <- c('motif_id')
...
# TSS +-10k
# Homo_sapiens.GRCh38.87.txt is written through extending +-10k to the TSS locations in the ENSEMBL annotation file Homo_sapiens.GRCh38.87.gtf
...
adult_brain_hts$queryHits <- tf_j[adult_brain_hts$queryHits,'motif_id']
adult_brain_hts$subjectHits <- grch38[adult_brain_hts$subjectHits, 'symbol' ]
adult_brain_hts <- unique(adult_brain_hts)
adult_brain_hts_h1 <- adult_brain_hts
#brain_hts <- brain_hts_o

save(adult_brain_hts_h1, file='your-working-directory/adult_brain/adult_brain_hts_h1.RData')
